...
## Classic Martini
To get up and running quickly, [martini.Classic()](http://godoc.org/github.com/scalingdata/go-martini#Classic) provides some reasonable defaults that work well for most web applications:
~~~ go
  m := martini.Classic()
...
Below is some of the functionality [martini.Classic()](http://godoc.org/github.com/scalingdata/go-martini#Classic) pulls in automatically:
...
The following services are included with [martini.Classic()](http://godoc.org/github.com/scalingdata/go-martini#Classic):
  ...
  * [martini.Route](http://godoc.org/github.com/scalingdata/go-martini#Route) - Current active route.
...
Route patterns may include named parameters, accessible via the [martini.Params](http://godoc.org/github.com/scalingdata/go-martini#Params) service:
~~~ go
...
Mapping on the request level can be done in a handler via [martini.Context](http://godoc.org/github.com/scalingdata/go-martini#Context):
~~~ go
...
A [martini.Classic()](http://godoc.org/github.com/scalingdata/go-martini#Classic) instance automatically serves static files from the "public" directory in the root of your server.
You can serve from more directories by adding more [martini.Static](http://godoc.org/github.com/scalingdata/go-martini#Static) handlers.
~~~ go
...
### Next()
[Context.Next()](http://godoc.org/github.com/scalingdata/go-martini#Context) is an optional function that Middleware Handlers can call to yield the until after the other Handlers have been executed. This works really well for any operations that must happen after an http request:
~~~ go
